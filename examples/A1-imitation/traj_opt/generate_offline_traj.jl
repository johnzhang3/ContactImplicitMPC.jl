#=
This script generate offline optimal trajectories from reference. this doesnt work
=#
...
function setup_problem(q_ref, h, T)
    q1 = first(q_ref);
    qT = last(q_ref);
    # q1 = q_ref[1]
    # qT = q_ref[39]
    ...
    # TODO: write option in a yaml file
    return x_ref, dyn, obj, cons, bnds

end

function save(x_sol, u_sol, gait, tol)
...
function solve_prob!(x_ref, dyn, obj, cons, bnds, T, gait)
    ...
    for tol in [1e-4, 1e-5, 1e-6]
        println("currently solving with tol $(tol)")
        p = DTO.Solver(dyn, obj, cons, bnds,
        options=DTO.Options(
            max_iter=2000,
            max_cpu_time = 300.0,
            ...
            @time DTO.solve!(p);  
            x_sol, u_sol = DTO.get_trajectory(p)  
            save(x_sol, u_sol, gait, tol)
    end
    return x_sol, u_sol
end

function main(visualize=false)
    # gaits = ["pace_forward"];
    # for gait in gaits
    #     q_ref, h, T = convert_q_from_json(gait)
    #     model, x_ref, dyn, obj, cons, bnds = setup_problem(q_ref, h, T)
    #     x_sol, u_sol = solve_prob!(x_ref, dyn, obj, cons, bnds, T, gait)
    # end

    gait = "pace_forward"
    q_ref, h, T = convert_q_from_json(gait)
    x_ref, dyn, obj, cons, bnds = setup_problem(q_ref, h, T)
    x_sol, u_sol = solve_prob!(x_ref, dyn, obj, cons, bnds, T, gait)
end
